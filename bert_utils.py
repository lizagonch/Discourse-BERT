...
class InputExample(object):
    ...
    def __init__(self, text, segment, cl, guid, label=None):#, text, segment, cl
        """Constructs a InputExample/

        Args:
            guid: Unique id for the example.
            text: string. The untokenized text of the sequence (it is the EDU in our case)
            label: defines the label encoding the rhetoric relation
            segment: Segment is the number defining to which of the text the current EDU belongs to
            cl: defines the class used for the final classification
        """
        self.guid = guid
        self.text_a = text
        self.label = label
...
class AugProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "train.tsv")), "train")  # train
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            # print(cl)
            examples.append(
                InputExample(guid=guid, text=text_a, segment=segment, cl=cl, label=label))
        return examples
    ...
    #        #print(cl)
    #        examples.append(
    #            InputExample(guid=guid, text=text_a, label=label))#, segment = 1, cl = 1, label=label))
    #    return examples
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # tokens:   [CLS] it was not rain ##ing [SEP]
        # type_ids: 0     0  0   0   0    0     0
        tokens_a = tokenizer._tokenize(example.text_a)
        tokens_label = label_map[example.label]
        tokens, init_ids, input_ids, input_mask, segment_ids, masked_lm_labels, segment_span, class_span = extract_features(
            tokens_a, tokens_label, example.segment, example.cl, max_seq_length, tokenizer)  #
        # , segment_span, class_span
        ...
        """print examples"""
        if ex_index < 5:
            logger.info("[bert_discourse] *** Example ***")
            logger.info("[bert_discourse] guid: %s" % (example.guid))
            logger.info("[bert_discourse] tokens: %s" % " ".join(
                [str(x) for x in tokens]))
            logger.info("[bert_discourse] init_ids: %s" % " ".join([str(x) for x in init_ids]))
            logger.info("[bert_discourse] input_ids: %s" % " ".join([str(x) for x in input_ids]))
            logger.info("[bert_discourse] input_mask: %s" % " ".join([str(x) for x in input_mask]))
            logger.info("[bert_discourse] segment_ids: %s" % " ".join([str(x) for x in segment_ids]))
            logger.info("[bert_discourse] masked_lm_labels: %s" % " ".join([str(x) for x in masked_lm_labels]))
    return features


def construct_train_dataloader(train_examples, label_list, max_seq_length, train_batch_size, num_train_epochs,
                               tokenizer, device):
    ...
    print(torch.tensor([f.init_ids for f in train_features], dtype=torch.long))
