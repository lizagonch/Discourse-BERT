...
def main():
    ...
    model = BertForSequenceClassification.from_pretrained(args.bert_model, output_hidden_states = True)

    if task_name == 'amazon':
        model.bert.embeddings.token_type_embeddings = torch.nn.Embedding(31, 768)
        model.bert.embeddings.token_type_embeddings.weight.data.normal_(mean=0.0, std=0.02)
    elif task_name == 'discourse':
        model.bert.embeddings.token_type_embeddings = torch.nn.Embedding(25, 768)
        model.bert.embeddings.token_type_embeddings.weight.data.normal_(mean=0.0, std=0.02)
    ...
    for e in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            _, input_ids, input_mask, segment_ids, masked_ids, segment_span, class_span = batch  # , segment_span, class_span
            """train generator at each batch"""
            # print(segment_span, class_span)
            optimizer.zero_grad()
            outputs = model(input_ids, input_mask, segment_ids, labels=class_span)
            loss = outputs.loss  # [0]
            loss.backward()
            avg_loss += loss.item()
            optimizer.step()
        ...
        if args.save_every_epoch:
            ...
            print("Saved name:", save_model_name)
